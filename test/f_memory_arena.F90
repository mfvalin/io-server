...
!     memadr = memory_arena_create_shared(shmid, NSYM, shmsz64)   ! allocate and initialize arena
  ...
  if(rank == 0) then
    ...
    call memory_arena_print_status(memadr);
    print 1,"id2, rank, address =", id2, rank,transfer(memadr,shmaddr)
    p = memory_block_create(memadr, DBLK*1, STR("BLOCK000"))
    p = memory_block_mark_init(memadr, STR("BLOCK000"))
    print 1,"BLOCK000 created at address",0, 0,transfer(p,shmaddr)
    p = memory_block_create(memadr, DBLK*2, STR("BLOCK001"))
    p = memory_block_mark_init(memadr, STR("BLOCK001"))
    print 1,"BLOCK001 created at address",0, 0,transfer(p,shmaddr)
    p = memory_block_create(memadr, DBLK*3, STR("BLOCK002"))
    p = memory_block_mark_init(memadr, STR("BLOCK002"))
    print 1,"BLOCK002 created at address",0, 0,transfer(p,shmaddr)
    p = memory_block_create(memadr, DBLK*4, STR("BLOCK003"))
    p = memory_block_mark_init(memadr, STR("BLOCK003"))
    print 1,"BLOCK003 created at address",0, 0,transfer(p,shmaddr)
    p = memory_block_create(memadr, DBLK*5, STR("BLOCK004"))
    p = memory_block_mark_init(memadr, STR("BLOCK004"))
  ...
  if(rank .ne. 0) then
    ...
    print 1,"attached, rank, shmid, addr =",rank, shmid, shmaddr
  endif
#endif
  ...
  p = memory_block_find_wait(memadr, bsize, bflags, STR("BLOCK000"), timeout)
  print 1,"BLOCK000 found at address",bsize, bflags,transfer(p,shmaddr)
  p = memory_block_find_wait(memadr, bsize, bflags, STR("BLOCK001"), timeout)
  print 1,"BLOCK001 found at address",bsize, bflags,transfer(p,shmaddr)
  p = memory_block_find_wait(memadr, bsize, bflags, STR("BLOCK002"), timeout)
  print 1,"BLOCK002 found at address",bsize, bflags,transfer(p,shmaddr)
  p = memory_block_find(     memadr, bsize, bflags, STR("BLOCK003"))
  print 1,"BLOCK003 found at address",bsize, bflags,transfer(p,shmaddr)
  p = memory_block_find(     memadr, bsize, bflags, STR("BLOCK004"))
  print 1,"BLOCK004 found at address",bsize, bflags,transfer(p,shmaddr)
777 continue
