...
module ioserver_timer_module
  ...
  type, public :: ioserver_timer
    ...
    contains
      ...
      procedure :: start
      procedure :: stop
      procedure :: get_total_time_ms
      procedure :: get_latest_time_ms
...
contains
  ...
  function is_valid(this)
    implicit none
    class(ioserver_timer), intent(in) :: this
    logical :: is_valid
  ...
  !> See IO_total_time_ms
  function get_total_time_ms(this) result(time)
    implicit none
    class(ioserver_timer), intent(in) :: this
    real(C_DOUBLE) :: time
    time = IO_total_time_ms(this % c_timer)
  end function get_total_time_ms

  !> See IO_latest_time_ms
  function get_latest_time_ms(this) result(time)
    implicit none
    class(ioserver_timer), intent(in) :: this
    real(C_DOUBLE) :: time
    time = IO_latest_time_ms(this % c_timer)
  end function get_latest_time_ms
