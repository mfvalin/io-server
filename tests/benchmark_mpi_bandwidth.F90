...
contains
  ...
  function run_test(msg_comm, msg_size, msg_count, window, target_channel, num_channels, offset, msg_buffer, &
    ...
    call MPI_Comm_rank(msg_comm, rank, ierror)
    call MPI_Comm_size(msg_comm, size, ierror)
...
program mpi_bandwidth
  ...
  if (on_server) then
    ...
    call MPI_Win_create(shared_buffer, MESSAGE_WINDOW_SIZE * total_num_relays, 4, MPI_INFO_NULL, msg_comm, window, ierror)
...
subroutine relay_process(msg_comm, msg_size, msg_count, window, target_channel, offset, msg_buffer)
  ...
  do i = 1, msg_count
    call MPI_Win_lock(LOCK_TYPE, target_channel, LOCK_ASSERT, window, ierror)
