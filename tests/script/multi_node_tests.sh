...
${MPI_COMMAND} -n ${NUM_SERVER_CPUS}   ${BUILD_PATH}/src/io_server_launch_server --put-cmd-timeout 5 --debug-level 1 : -n ${NUM_MODEL_CPUS} ${BUILD_PATH}/tests/io_server_many_streams || exit -1
${MPI_COMMAND} -n ${NUM_SERVER_CPUS}   ${BUILD_PATH}/src/io_server_launch_server --max-streams 100 --debug-level 1   : -n ${NUM_MODEL_CPUS} ${BUILD_PATH}/tests/io_server_many_streams || exit -1
