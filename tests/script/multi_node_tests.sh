...
${MPI_COMMAND} -n ${NUM_MODEL_CPUS} ${BUILD_PATH}/tests/io_server_launch_pseudo_model : -n ${NUM_SERVER_CPUS} ${BUILD_PATH}/tests/io_server_launch_server      || exit -1
${MPI_COMMAND} -n ${NUM_SERVER_CPUS} ${BUILD_PATH}/tests/io_server_launch_server      : -n ${NUM_MODEL_CPUS} ${BUILD_PATH}/tests/io_server_model_integration   || exit -1
${MPI_COMMAND} -n ${NUM_MODEL_CPUS} ${BUILD_PATH}/tests/io_server_model_integration   : -n ${NUM_SERVER_CPUS} ${BUILD_PATH}/tests/io_server_launch_server      || exit -1
