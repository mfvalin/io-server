...
#include <serializer.hf>
    ...
    block
      ...
      type(grid_bounds_t) :: halo_grid, local_grid
      ...
      call global_grid % set(min_bound = grid_index_t(),                                                        &
                             size      = grid_index_t(global_num_x, global_num_y))
      call local_grid % set (min_bound = grid_index_t(tile_id_x * base_num_x + 1, tile_id_y * base_num_y + 1),  &
                             size      = grid_index_t(base_num_x, base_num_y))
      local_grid = local_grid % intersection(global_grid)
      call halo_grid % set(min_bound = grid_index_t((/1, 1/) - halo_size),                                      &
                           size      = grid_index_t(local_grid % get_size_val() + (/2 * halo_size, 2*halo_size, 0, 0, 0/)))
      ...
      ! real_tile_size = halo_grid % get_size()
      real_num_x = halo_grid % get_size(1)
      real_num_y = halo_grid % get_size(2)

      data_info = node_heap % allocate(data_array_tmp, [real_num_x, real_num_y])

      if (.not. associated(data_array_tmp)) then
        print '(A, 1x, A)', context % get_short_pe_name(), 'ERROR: Could not allocate from shmem heap!'
        return
      end if

      data_array(-halo_size+1 : real_num_x - halo_size, -halo_size+1 : real_num_y - halo_size) => data_array_tmp

      if (size(data_array) .ne. size(data_array_tmp)) then
        print '(A, 1X, A, I12, I12)', context % get_short_pe_name(), 'ERROR: We have a problem, size of arrays do not match', size(data_array), size(data_array_tmp)
        return
      end if

      ! print *, 'lbound: ', lbound(data_array), ', ubound: ', ubound(data_array)

      do j = lbound(data_array, dim = 2), ubound(data_array, dim = 2)
        g_j = j + local_grid % get_min(2) - 1
        do i = lbound(data_array, dim = 1), ubound(data_array, dim = 1)
          g_i = i + local_grid % get_min(1) - 1
          data_array(i, j) = compute_int4(g_i, g_j, 1)
        end do
      end do
      ! data_array(:,:) = 1
      ! data_array_tmp(1:6, 1) = [1, 0, 2, 0, 5, 0]
      ! data_array_tmp(real_num_x - 6:real_num_x, real_num_y) = [4, 0, 3, 0, 6, 0]
      ! data_array_tmp(:,:) = 0
      ...
      success = output_stream % send_data(data_info, halo_grid, local_grid, global_grid, reduced_global_grid, command = command_jar) .and. success
